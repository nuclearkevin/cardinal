...
BooleanComboHeuristicUserObject::validParams(){
    ...
    params.addRequiredParam<std::vector<ExtraElementIDName>>("extra_ids"," extra integer ids of the other heuristics");
    params.addRequiredParam<std::vector<std::string>>("boolean_logic"," order of the boolean logic applied to the user objects");
    params.addClassDescription("Takes different heuristic user objacts "
                               " and applies user defined boolean logic");
    return params;
}

BooleanComboHeuristicUserObject::BooleanComboHeuristicUserObject(const InputParameters& params):
        ClusteringUserObject(params),
        _parsed_user_object_extra_element_id_names(getParam<std::vector<ExtraElementIDName>>("extra_ids")),
        _boolean_logic(getParam<std::vector<std::string>>("boolean_logic"))
{
    ...
    for (auto id_name: _parsed_user_object_extra_element_id_names) {
        ...
        {
            mooseError("Mesh does not have an extra element integer named ",id_name, "."
    ...
    if (size(_parsed_user_object_extra_element_id_names)-1 !=size(_boolean_logic) ){
        mooseError("not suficient/more or less number of logics provided than needed " );
    }
...
bool BooleanComboHeuristicUserObject::passesTheComboLogic(libMesh::Elem* element){

    /// neighbor element is not need basically
    ...
    ///now I have to create something like a logic dot product
